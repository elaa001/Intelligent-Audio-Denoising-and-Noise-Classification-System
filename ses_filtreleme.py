...
def analyze_and_plot_signal(signal, fs, title="Signal Analysis", output_folder="grafikler", filename_prefix="plot"):
    ...
    plt.title(f"{title} - Guc Spektral Yogunlugu")
    ...
    print(f"\U0001F4CA '{filename_prefix}' icin grafikler '{output_folder}/' klasorune kaydedildi (PSD ve spektrogram).")
...
def process_and_clean(original_path, noisy_path, output_path):
    ...
    analyze_and_plot_signal(noisy, fs_n, title="Gurultulu Sinyal", filename_prefix=os.path.basename(noisy_path).split('.')[0])
    ...
    analyze_and_plot_signal(denoised, fs_o, title="Temizlenmis Sinyal", filename_prefix=os.path.basename(output_path).split('.')[0])

    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    wavfile.write(output_path, fs_o, (denoised * 32767).astype(np.int16))
    print(f" {output_path} basariyla kaydedildi. (Algilanan Gurutu Turu: {noise_type})")
